...
module.exports = kconfig = async (kill, message) => {
    ...
    try {
		...
		case 'downaudio':
            ...
            try {
				await kill.reply(from, mess.wait(), id)
				await youtubedl(`${url}`, { noWarnings: true, noCallHome: true, noCheckCertificate: true, preferFreeFormats: true, youtubeSkipDashManifest: true, referer: `${url}`, x: true, audioFormat: 'mp3', o: `./lib/media/audio/${lvpc}.mp3` }).then(async () => { await kill.sendPtt(from, `./lib/media/audio/d${user.replace('@c.us', '')}${lvpc}.mp3`, id) })
				await sleep(10000).then(async () => { await fs.unlinkSync(`./lib/media/audio/${lvpc}.mp3`) })
			} catch (error) {
				await kill.reply(from, mess.verybig(), id)
